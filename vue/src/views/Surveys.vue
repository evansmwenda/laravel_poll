<template>
  <PageComponent>
    ...
    <form v-else @submit.prevent="savePoll" class="animate-fade-in-down">
...
// Get poll loading state, which only changes when we fetch poll from backend
...
// Watch to current poll data change and when this happens we update local model
watch(
...
// If the current component is rendered on poll update route we make a request to fetch survey
if (route.params.id) {
...
function savePoll() {
  ...
  store.dispatch("savePoll", { ...model.value }).then(({ data }) => {
    store.commit("notify", {
      type: "success",
      message: "The poll was successfully " + action,
    });
    router.push({
      name: "SurveyView",
